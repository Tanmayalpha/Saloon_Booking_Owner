...
class Ui {
  static GetSnackBar SuccessSnackBar({String title = 'Success', String message, SnackPosition snackPosition = SnackPosition.BOTTOM}) {
    ...
    return GetSnackBar(
      ...
      mainButton: TextButton(
        ...
        child: Text("OK", style: Get.textTheme.headline6.merge(TextStyle(color: Get.theme.primaryColor))),
      ),
  ...
  static Widget getPrice(double myPrice, {TextStyle style, String zeroPlaceholder = '-', String unit,bool from = false}) {
      ...
      return RichText(
        ...
        text: _setting.setting.value.currencyRight != null && _setting.setting.value?.currencyRight == false
            ? TextSpan(
                ...
                style: getPriceStyle(style,from),
                children: <TextSpan>[
                  ...
                  if (unit != null) TextSpan(text: " " + unit + " ", style: getPriceStyle(style,from)),
                ],
              )
            : TextSpan(
                ...
                children: <TextSpan>[
                  TextSpan(text: _setting.setting.value?.defaultCurrency, style: getPriceStyle(style,from)),
                  if (unit != null) TextSpan(text: " " + unit + " ", style: getPriceStyle(style,from)),
                ],
              ),
      );
  ...
  static TextStyle getPriceStyle(TextStyle style,bool from) {
    if (style == null) {
      return Get.textTheme.subtitle2.merge(
        TextStyle(fontWeight: FontWeight.w300, fontSize: from?14.0:Get.textTheme.subtitle2.fontSize - 4),
      );
    } else {
      return style.merge(TextStyle(fontWeight: FontWeight.w300, fontSize: from?14.0:style.fontSize - 4));
    }
  }
  ...
  static Future<String> showDatePickerDialog(BuildContext context, String initialDate) async {
  ...
  static DateFormatString(String picked){
    return DateFormat("hh:mm a").format(DateTime(2000,12,12,int.parse(picked.split(":")[0]),int.parse(picked.split(":")[1])));
  }
