...
class ServicesListWidget extends GetView<EServicesController> {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      if (Get.find<LaravelApiClient>().isLoading(tasks: ['getEServices', 'getFeaturedEServices', 'getPopularEServices', 'getAvailableEServices']) &&
      ...
      } else if (controller.category.isEmpty||controller.category[controller.catIndex.value].eServices.isEmpty) {
        ...
        return ListView.builder(
          ...
          primary: false,
          shrinkWrap: true,
          itemCount: controller.category[controller.catIndex.value].eServices.length,
          itemBuilder: ((_, index) {
            var _service = controller.category[controller.catIndex.value].eServices.elementAt(index);
            return ServicesListItemWidget(service: _service);
            if (index == controller.category[controller.catIndex.value].eServices.length) {
              return Obx(() {
                return Padding(
