...
class SalonController extends GetxController {
  ...
  GlobalKey bank = GlobalKey(),service = GlobalKey(),
      link = GlobalKey(),
      working = GlobalKey(),
      edit = GlobalKey();
  SalonController() {
 ...
 //   heroTag = arguments['heroTag'] as String;
  ...
  Future refreshSalon({bool showMessage = false}) async {
    ...
    if (showMessage) {
      Get.showSnackbar(Ui.SuccessSnackBar(
          message: salon.value.name + " " + "page refreshed successfully".tr));
    }
  }
  ...
  Future getFeaturedEServices() async {
    try {
      featuredEServices.assignAll(await _salonRepository.getFeaturedEServices(
          salonId: salon.value.id, page: 1));
    } catch (e) {
      Get.showSnackbar(Ui.ErrorSnackBar(message: e.toString()));
    }
  }
  ...
  Future getExperiences() async {
    try {
      experiences
          .assignAll(await _salonRepository.getExperiences(salon.value.id));
    } catch (e) {
      Get.showSnackbar(Ui.ErrorSnackBar(message: e.toString()));
    }
  }
